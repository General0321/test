...
public class PairEditorDialog extends JDialog {
     ...
     * 创建简化的高级功能面板
     */
    private JPanel createSimplifiedAdvancedPanel() {
        JPanel panel = new JPanel(new BorderLayout(10, 10));
        ...
        JLabel titleLabel = new JLabel("高级功能");
        titleLabel.setFont(titleLabel.getFont().deriveFont(Font.BOLD, 14f));
        headerPanel.add(titleLabel, BorderLayout.NORTH);
        
        JLabel descLabel = new JLabel(
            "💡 仅在使用多个配对(Pair)时需要配置以下功能，单配对场景可忽略"
        );
        descLabel.setFont(descLabel.getFont().deriveFont(Font.PLAIN, 11f));
        descLabel.setForeground(new Color(100, 100, 100));
        descLabel.setBorder(BorderFactory.createEmptyBorder(5, 0, 0, 0));
        ...
        // 中间内容：使用标签页组织
        JTabbedPane advTabbedPane = new JTabbedPane();
        
        // 1. 响应对比
        comparisonPanel = new ResponseComparisonConfigPanelSimplified(pair.getComparisonConfig());
        advTabbedPane.addTab("📊 响应对比", comparisonPanel);
        advTabbedPane.setToolTipTextAt(0, "对比多个Pair的响应特征");
        
        // 2. 变量提取
        advTabbedPane.addTab("📝 变量提取", createVariableExtractionPanel());
        advTabbedPane.setToolTipTextAt(1, "从响应中提取数据供后续使用");
        
        panel.add(advTabbedPane, BorderLayout.CENTER);
     ...
     * ✨ 新增：创建变量提取面板
     */
    private JPanel createVariableExtractionPanel() {
        JPanel panel = new JPanel(new BorderLayout(10, 10));
        ...
        JLabel titleLabel = new JLabel(
            "变量提取配置 - 从当前Pair的响应中提取变量，供后续Pair的请求或响应条件使用"
        );
        headerPanel.add(titleLabel, BorderLayout.NORTH);
        
        JLabel exampleLabel = new JLabel(
            "示例： 变量名: user_id, 正则表达式: \"user_id\":\\s*\"([^\"]+)\", 后续Pair使用: {user_id}"
        );
        headerPanel.add(exampleLabel, BorderLayout.CENTER);
        panel.add(headerPanel, BorderLayout.NORTH);
        
        // 中间：变量提取表格
        String[] columns = {"变量名", "正则表达式", "操作"};
        variableTableModel = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return column != 2; // 操作列不可编辑
            }
        ...
        // 添加删除按钮到操作列
        variableTable.getColumnModel().getColumn(2).setCellRenderer((table, value, isSelected, hasFocus, row, column) -> {
            JButton deleteButton = new JButton("删除");
            deleteButton.setMargin(new Insets(2, 5, 2, 5));
            return deleteButton;
        });
        
        variableTable.getColumnModel().getColumn(2).setCellEditor(new javax.swing.table.TableCellEditor() {
            private JButton button = new JButton("删除");
            private int currentRow;
            
            {
                button.addActionListener(e -> {
                    variableTableModel.removeRow(currentRow);
                    fireEditingStopped();
                });
            }
            ...
            @Override
            public boolean shouldSelectCell(java.util.EventObject e) { return false; }
            @Override
        ...
        JButton addButton = new JButton("➕ 添加变量");
        addButton.addActionListener(e -> {
            String varName = JOptionPane.showInputDialog(this, "请输入变量名:", "添加变量", JOptionPane.PLAIN_MESSAGE);
            if (varName != null && !varName.trim().isEmpty()) {
                ...
                if (regex != null && !regex.trim().isEmpty()) {
                    variableTableModel.addRow(new Object[]{varName.trim(), regex.trim(), "删除"});
                }
            }
        });
        buttonPanel.add(addButton);
     ...
     * 简化的帮助面板
     */
    private JPanel createSimplifiedHelpPanel() {
        JPanel panel = new JPanel(new BorderLayout(10, 10));
        panel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
        
        // 创建HTML内容
        JEditorPane helpPane = new JEditorPane("text/html", getSimplifiedHelpHTML());
     ...
     * 获取简化的帮助HTML
     */
    private String getSimplifiedHelpHTML() {
        return "📖 快速入门指南\n\n" +
            "═══════════════════════════════════════\n\n" +
            "🎯 什么是配对？\n" +
            "配对 = 请求条件 + 响应规则\n" +
            "  • 请求条件：哪些请求会被测试？在哪里注入payload？\n" +
            "  • 响应规则：如何判断漏洞存在？检查什么特征？\n\n" +
            "═══════════════════════════════════════\n\n" +
            "⚡ 三步配置\n\n" +
            "步骤 1：配置请求条件\n" +
            "  1. 点击「1️⃣ 请求条件」标签页\n" +
            "  2. 添加HTTP元素（如Parameter、Header等）\n" +
            "  3. 勾选「匹配」= 这是请求条件\n" +
            "  4. 勾选「注入」= 这是注入点\n" +
            "  5. 填写Payload（如：{{ORIGINAL}}'）\n\n" +
            "步骤 2：配置响应规则\n" +
            "  1. 点击「2️⃣ 响应匹配」标签页\n" +
            "  2. 添加响应元素（如Status Code、Body等）\n" +
            "  3. 设置匹配规则（如：包含\"error\"）\n" +
            "  4. 保存配置\n\n" +
            "步骤 3：高级配置（可选）\n" +
            "  点击「⚙️ 高级配置」标签页可设置：\n" +
            ...
            "💡 示例：SQL注入检测\n\n" +
            "配对名称：SQL错误注入检测\n" +
            "请求条件：Parameter: id (匹配+注入)\n" +
            "          Payload: {{ORIGINAL}}'\n" +
            "响应规则：Body 包含: sql, mysql, error\n\n" +
            "═══════════════════════════════════════\n\n" +
            "❓ 常见问题\n\n" +
            "Q: \"匹配\" 和 \"注入\" 有什么区别？\n" +
            "A:\n" +
            "  • 匹配：这个元素必须满足条件（如：Method=POST）\n" +
            "  • 注入：在这个位置插入payload（如：在id参数注入'）\n\n" +
            "Q: 什么时候需要\"高级配置\"？\n" +
            "A: 在以下场景：\n" +
            "  • 时间盲注（需要对比响应时间）\n" +
            "  • 布尔盲注（需要对比两次响应内容）\n" +
            "  • 多步骤检测（需要从第1个响应提取数据用于第2个请求）\n" +
            "  普通检测使用默认设置即可！\n\n" +
            "═══════════════════════════════════════\n\n" +
            "💡 提示：先配置简单的规则测试，熟悉后再使用高级功能";
    }
